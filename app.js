...
app.get('/results', (req, res) => {
  ...
  if (dayNumber < 1 || dayNumber > 5) {
    res.send(`
      <html>
        ...
        <body>
          <h1>No hay encuesta activa hoy.</h1>
          <p><a href="/">Volver al inicio</a></p>
        </body>
      </html>
    `);
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .come-sano th { background-color: #27ae60; }
          .indigo th { background-color: #e67e22; }
          .new th { background-color: #8e44ad; }
          a { 
            text-decoration: none; 
            color: #2980b9; 
            font-weight: bold;
  ...
  ["1","2","3","4","5"].forEach(option => {
    const comidaCS = weeklyPollData[dayName]["ComeSano"][option] || "Arma tu Ensalada";
  ...
  // Si es viernes, agregamos la tabla para NEW
  if(dayName === 'viernes'){
    resultHTML += `
        <!-- Tabla para NEW 😎 -->
        <table class="new">
          ...
          <tbody>
    `;
    // Opciones fijas para NEW
    const fridayNewOptions = ["Asado", "Dubai", "Empanadas", "Napoelon", "5marias"];
    ["1","2","3","4","5"].forEach(option => {
      const votosNew = (currentVotes[option]?.["NEW"] || []).length;
      resultHTML += `
        <tr>
          <td>${fridayNewOptions[parseInt(option) - 1]}</td>
          <td>${votosNew}</td>
        </tr>
      `;
    });
...
// Descomenta para pruebas inmediatas
client.on('ready', () => {
...
function sendPoll() {
  ...
  const dayName = dayNames[dayNumber];
  let pollText = `Elige la comida para hoy ${dayName}:\n\n`;

  if (dayName === 'viernes') {
    // Sección COME SANO (5 opciones)
    pollText += "ComeSano🥙:\n";
    for (let i = 1; i <= 5; i++) {
      pollText += `${i}. ${weeklyPollData[dayName]["ComeSano"][i]}\n`;
    }
    pollText += "\nIndigo🥫:\n";
    // Sección INDIGO (3 opciones)
    for (let i = 1; i <= 3; i++) {
      pollText += `${i}. ${weeklyPollData[dayName]["Indigo"][i]}\n`;
    }
    pollText += "\nNEW 😎:\n";
    const fridayNewOptions = ["Asado🔥", "Dubai🧞", "Empanadas🥟", "Napoelon👴🏼", "5marias👩‍🦳"];
    fridayNewOptions.forEach((option, index) => {
      pollText += `${index + 1}. ${option}\n`;
    });
    pollText += "\nElige el lugar:\n";
    pollText += "C: ComeSano | I: Indigo | N: Nuevas Opciones\n\n";
    pollText += "Responde con el número de la opción y la letra del lugar, todo junto.\nEjemplo: \"1C\"\n";
    pollText += "Para Armar tu ensalada: \"5C\"";
  } else {
    // Para otros días (formato tradicional)
    pollText += "ComeSano:\n";
    for (let i = 1; i <= 5; i++) {
      pollText += `${i}. ${weeklyPollData[dayName]["ComeSano"][i]}\n`;
    }
...
client.on('message', message => {
  ...
  if (dayName === 'viernes') {
    // Permitir formato sin espacio: ej. "1C", "2I", "3N" (acepta mayúsculas o minúsculas)
    const voteMatch = message.body.trim().match(/^([1-5])\s*([CINcin])$/);
    if (voteMatch) {
      const option = voteMatch[1]; // "1" a "5"
      const letter = voteMatch[2].toUpperCase();
      let group;
      if (letter === 'C') {
        group = 'ComeSano';
      } else if (letter === 'I') {
        group = 'Indigo';
        if (parseInt(option) > 3) { // Solo 3 opciones para Indigo
          message.reply("Opción inválida para Indigo.");
          return;
        }
      } else if (letter === 'N') {
        group = 'NEW';
      }
      // Asegurarse de que para la opción, en caso de voto NEW, exista la propiedad NEW
      if (group === 'NEW' && !votes[dayName][option].hasOwnProperty("NEW")) {
        votes[dayName][option]["NEW"] = [];
      }
      // Elimina votos previos de este usuario para la opción en todos los grupos
      Object.keys(votes[dayName][option]).forEach(g => {
        votes[dayName][option][g] = votes[dayName][option][g].filter(id => id !== voterId);
      });
      votes[dayName][option][group].push(voterId);
      saveVotes();
      
      let selectedFood = "";
      if (group === 'NEW') {
        const fridayNewOptions = ["Asado", "Dubai", "Empanadas", "Napoelon", "5marias"];
        selectedFood = fridayNewOptions[parseInt(option) - 1];
      } else {
        selectedFood = weeklyPollData[dayName][group][option];
      }
      client.sendMessage(voterId, `Tu voto para "${selectedFood}" en ${group} ha sido registrado.`);
      console.log(`Votos para ${dayName}:`, votes[dayName]);
      return;
    }
  }
  
  // Para días que no sean viernes o para mensajes que sigan el formato tradicional
  const parts = message.body.trim().split(/\s+/);
  if (parts.length === 2 && /^[1-5]$/.test(parts[0]) && /^[CIci]$/.test(parts[1])) {
